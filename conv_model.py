...
def create_dense_dropout_model(optimizer='adagrad', kernel_initializer='glorot_uniform', dropout=0.2):
  model = tf.keras.models.Sequential([
      tf.keras.layers.Embedding(vocab_size,16,input_length=120), # input embedding learnt of length 16
      tf.keras.layers.Conv1D(32, 3, activation='relu'),
      tf.keras.layers.Flatten(),
...
def create_stacked_model(optimizer='adagrad', kernel_initializer='glorot_uniform', dropout=0.2):
  model = tf.keras.models.Sequential([
      tf.keras.layers.Embedding(vocab_size,16,input_length=120), # input embedding learnt of length 16
      tf.keras.layers.Conv1D(32, 3, activation='relu'),
      tf.keras.layers.Conv1D(32, 3, activation='relu'),
...
model4 = KerasClassifier(build_fn=create_stacked_model,epochs = 10,verbose=1)

models = [model4, model1, model2, model3]
names = ["Conv Stacked", "Conv", "Conv with dropout", "Conv with Batch Norm"]
...
for i in range(len(models)):
  pipe = Pipeline([('Convert to lower', ConvertToLower('Data')),
                  ...
                  ('Pad Sequences', PadSequences('sequences')),
                  ('Model', models[i])
                  ])
